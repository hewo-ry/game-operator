...
def create_fn(spec, name, namespace, logger, **kwargs):
    ...
    if data_pvc_name not in pvc_names:
        # Templating
        data_pvc = PersistentVolumeClaim.from_spec(spec.get("storage")).to_pvc(data_pvc_name, namespace, common_labels)
    ...
    if config_map_name not in config_map_names:
        data_config_map = MinecraftServerConfig.from_spec(spec.get("serverConfig")).to_config_map(config_map_name, namespace, common_labels)
        ...
        obj_config_map = core_api.create_namespaced_config_map(
            namespace=namespace,
            body=data_config_map
        )
        logger.info(f"ConfigMap {obj_config_map.metadata.name} created in namespace {namespace}.")
    ...
    if secret_name not in secret_names:
        data_secrets = MinecraftServerSecrets().to_secret(secret_name, namespace, common_labels)
        ...
        logger.info(f"Secret {obj_secrets.metadata.name} created in namespace {namespace}.")
    ...
    if service_name not in service_names:
        data_service = Service(ports=[
            NodePort(
                "minecraft", 25565, Protocol.TCP, "minecraft"
            ),
            NodePort(
                "query", 25565, Protocol.UDP, "query"
            )
        ...
        logger.info(f"Service {obj_service.metadata.name} created in namespace {namespace}.")
    ...
    if deployment_name not in deployment_names:
        data_deployment = DeploymentConfig.from_spec(name, namespace, spec.get("serverConfig")).to_deployment(common_labels)
        ...
        obj_deployment = apps_api.create_namespaced_deployment(
            namespace=namespace,
            body=data_deployment
        )
        logger.info(f"Deployment {obj_deployment.metadata.name} created in namespace {namespace}.")
...
def relabel(diff, status, namespace, logger, **kwargs):
    ...
    logger.info(f"ConfigMap {config_map_obj.metadata.name} labels updated in namespace {namespace}.")
    secret_obj = api.patch_namespaced_secret(
        namespace=namespace,
        name=secret_name,
        body=patch
    )
    logger.info(f"Secret {secret_obj.metadata.name} labels updated in namespace {namespace}.")
